...
Initialize(int argc, char **argv)
{
    ...
    freePagesMap = new Bitmap(NUM_PHYS_PAGES);
    ...
    fileSynchList = new FileSynchList();
#endif
...
#ifdef FILESYS
    char rootPath[2] = {'/', '\0'};
    currentThread->SetCurrentDir(fileSystem->FindDirectory(rootPath));
    if (!format)
        fileSystem->Cleanup();
#endif
}
...
#ifdef FILESYS
    while (!fileSynchList->IsEmpty()) {
        FileSynch *fileSynch = fileSynchList->Pop();
        delete fileSynch;
    }
    delete fileSynchList;
    delete synchDisk;
#endif
