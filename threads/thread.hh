...
public:
    ...
    /// Initialize a `Thread`.
    Thread(const char *threadName, bool canJoin, unsigned threadPriority);
    ...
    /// Set a new priority for this thread.
    void SetNewPriority(unsigned newPriority);

    /// Restore this thread's previous priority.
...
private:
    ...
    /// A channel used to send and receive this thread's exit status.
    Channel *joinChannel;

    /// The priority assigned to this thread.
    unsigned priority;

    /// A backup for this thread's priority, in case it has to be temporarily
    /// changed.
    int priorityBackup;
    ...
    /// A table holding the files this thread has opened.
...
public:
    ...
    /// Add file to the `openedFiles` table.
    OpenFileId AddFile(OpenFile *file);

    /// Get file with the given OpenFileId from the `openedFiles` table.
    OpenFile *GetFile(OpenFileId fileId);

    /// Remove file with the given OpenFileId from the `openedFiles` table.
    OpenFile *RemoveFile(OpenFileId fileId);
...
#ifdef FILESYS
    /// Set this thread's current working directory.
    void SetCurrentDir(DirSynch *dirSynch);

    /// Get this thread's current working directory.
    DirSynch *GetCurrentDir();

private:

    /// Current working directory for this thread.
    DirSynch *currentDir;
#endif
};
