...
/// Change `thread` priority to `newPriority`, and change it to it's new
/// priority's ready list.
///
/// * `thread` is the thread to be changed.
/// * `newPriorty` is the thread's new priority.
void
Scheduler::ChangeThreadPriority(Thread *thread, unsigned newPriority)
{
