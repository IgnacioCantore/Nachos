...
Thread::Thread(const char *threadName, bool canJoin, unsigned threadPriority)
{
    ASSERT(threadPriority < MAX_PRIORITY);
    ...
    openedFiles->Add(nullptr);  // File descriptor for stdin.
    openedFiles->Add(nullptr);  // File descriptor for stdout.
...
Thread::~Thread()
{
    ...
    if (space != nullptr) {
        delete space;
        if (spaceId != 0)    // Only deallocate name for threads other
            delete [] name;  // than `main`.
    }
    delete openedFiles;
#endif
}
...
/// Set this thread's status to `st`.
void
...
/// If this thread can be joined, wait until it finishes and return its exit
/// status.
int
Thread::Join()
{
...
/// Backup this thread's priority and change it.
void
Thread::SetNewPriority(unsigned newPriority)
{
...
/// Restore this thread's priority to its old value.
void
Thread::RestorePriority()
{
    if (priorityBackup != -1) {
        scheduler->ChangeThreadPriority(this, priorityBackup);
...
/// Add `file` to the `openedFiles` table and return its OpenFileId.
OpenFileId
...
/// Get file with `fileId` from the `openedFiles` table.
OpenFile *
...
/// Remove file with `fileId` from the `openedFiles` table and return the file.
OpenFile *
...
#ifdef FILESYS
/// Set this thread's current working directory to `dirSynch`.
void
Thread::SetCurrentDir(DirSynch *dirSynch)
{
    ASSERT(dirSynch != nullptr);
    currentDir = dirSynch;
}

/// Get this thread's current working directory.
DirSynch *
