...
bool CountArgsToSave(int address, unsigned *count)
{
    ...
    unsigned c = 0;
    do {
        for (int j = 0; !(machine->ReadMem(address + 4 * c, 4, &val)) &&
             j < MAX_PAGE_FAULTS; j++);
        c++;
...
SaveArgs(int address)
{
    ...
    for (unsigned i = 0; i < count; i++) {
        ...
        for (int j = 0; !(machine->ReadMem(address + i * 4, 4, &strAddr)) &&
             j < MAX_PAGE_FAULTS; j++);
...
WriteArgs(char **args)
{
    ...
    for (unsigned i = 0; i < c; i++)
        for (int j = 0; !(machine->WriteMem(sp + 4 * i, 4, argsAddress[i])) &&
             j < MAX_PAGE_FAULTS; j++);
    for (int j = 0; !(machine->WriteMem(sp + 4 * c, 4, 0)) &&
         j < MAX_PAGE_FAULTS; j++);  // The last is null.
