...
public:
    ...
    /// Remove first occurence of item from the list.
    ...
    /// Get the first item with the given `key`
    Item Get(int key);
...
/// Remove first occurence of an item from the list.
///
/// * `item` is the item to be removed from the list.
template <class Item>
void
...
///
/// * `item` is what to check if it's on the list.
template <class Item>
bool
...
/// Returns a pointer to the first item with the given `key`, `Item()` if
/// there is none on the list.
template <class Item>
Item
