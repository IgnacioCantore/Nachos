...
OpenFile::OpenFile(int sector)
{
    ...
    if (fileSynch != nullptr)
        hdr = fileSynch->GetFileHeader();
    else {  // Directories and the bit map of free disk blocks.
        hdr = new FileHeader;
...
/// Also check if the file is pending of removal. If so, call file system
/// operation `Remove`.
OpenFile::~OpenFile()
{
    fileSynchList->AcquireLock();
    FileSynch *fileSynch = fileSynchList->Get(hdrSector);
    if (fileSynch != nullptr) {
        bool allClosed = fileSynch->FileClosed();
        if (allClosed)  // All threads have closed this file.
            fileSynchList->Remove(fileSynch);  // Remove its synch structure
                                               // from the list.
        fileSynchList->ReleaseLock();
        if (fileSynch->ReadyToRemove())
            fileSystem->Remove(fileSynch->GetFilePath());
        if (allClosed)
            delete fileSynch;
    } else {
        fileSynchList->ReleaseLock();
...
OpenFile::WriteAt(const char *from, unsigned numBytes, unsigned position)
{
    ...
    unsigned moreBytes = position + numBytes;
    unsigned newBytes = moreBytes - fileLength;

    if (moreBytes > fileLength && !fileSystem->ExpandFile(hdrSector, newBytes))
