...
public:
    ...
    bool Create(const char *path, unsigned initialSize, bool isDir = false);
    ...
    /// Expand a file in `newBytes`.
    bool ExpandFile(unsigned sector, unsigned numBytes);

    /// Split a path string into a directory path and the file/directory name.
    void SplitPath(const char *path, char *dirPath, char *name);

    /// Find a directory, given its path, and return its DirSynch.
    FSSynch *FindDirectory(char *dirPath);

    /// Clean SWAP files.
    void Cleanup();

    /// List all the files in the file system (UNIX `ls -R`).
    void List();
...
private:
    FSSynchList *fsSynchList;  ///< List of synchronization structures to
                               ///< ensure mutual exclusion for the bit map of
                               ///< free disk blocks and for directories.
};
